...
function JsSuggestSale(oHandler, sParams, sParser, domain, ssubmit)
{
	...
	{
		t.Destroy();
        var relativeBlock = ($("#" + t.oObj.id).parent().hasClass('city-block-content')) ? true : false;
        if (relativeBlock === true) {
           t.oDiv = t.oObj.parentNode.appendChild(document.createElement("DIV"));
        } else {
        ...
        if (relativeBlock === true) {
            TCJsUtils.show(t.oDiv, 0, 45);
            if (result.length > 3) {
                t.oDiv.style.top = "auto";
                t.oDiv.style.bottom = "45px";
            }
        } else {
	...
	t.Replace = function()
	{
		if (typeof t.oActive == 'string')
		{
			...
			if (typeof tmp == 'object')
			{
				...
				elEntities.innerHTML = TCJsUtils.htmlspecialcharsback(tmp['CITY_NAME']);
