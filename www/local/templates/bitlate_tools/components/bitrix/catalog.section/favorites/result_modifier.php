...
$arResult['ITEMS_SUB'] = $arResult['ITEMS'];
$arResult['ITEMS'] = array();
foreach ($arResult['ITEMS_SUB'] as $arItem) {
    if (isset($arItem['OFFERS']) && !empty($arItem['OFFERS'])) {
        foreach ($arItem['OFFERS'] as $arOffer) {
            if ($arParams['USER_FAVORITES'][$arItem['ID']][$arOffer['ID']] > 0) {
                if (!$arOffer['PREVIEW_PICTURE']) {
                    $arOffer['PREVIEW_PICTURE'] = ($arItem['PRODUCT_PREVIEW']) ? $arItem['PRODUCT_PREVIEW'] : $arItem['PREVIEW_PICTURE'];
                }
                $arOffer['DETAIL_PAGE_URL'] = $arItem['DETAIL_PAGE_URL'];
                $arOffer['PROPERTIES']['OLD_PRICE'] = $arItem['PROPERTIES']['OLD_PRICE'];
                $arOffer['IS_OFFER'] = "Y";
