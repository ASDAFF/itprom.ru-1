        ...
        <?foreach ($arResult['ITEMS'] as  $arItem):
            if (intval($arItem['PREVIEW_PICTURE'])):
                ...
                <div class="item vertical-middle" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
                    <<?=$blockSelector?><?if ($detailUrl):?> href="<?=$arItem["CODE"]//=$detailUrl?>"<?endif;?> class="background <?=($arItem['PROPERTIES']['STYLE']['VALUE_XML_ID'] == 'white') ? 'white' : 'black'?>" style="background-image:url(<?=$pic['src']?>);">
                        <span class="container row">
                            <? if($arItem['PROPERTIES']['SHOW_TITLE']['VALUE'] != "Нет"){?>
                                <span class="main-slider-caption"><?=$arItem['NAME']?></span>
                            <?}?>
                            <span class="main-slider-desc"><?=$arItem['PREVIEW_TEXT']?></span>

                            <? if($arItem['PROPERTIES']['SHOW_BTN']['VALUE'] != "Нет"){?>
