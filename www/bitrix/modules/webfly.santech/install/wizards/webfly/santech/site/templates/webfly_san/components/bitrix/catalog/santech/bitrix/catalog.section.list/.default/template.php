...
if ('Y' == $arParams['SHOW_PARENT_NAME'] && 0 < $arResult['SECTION']['ID'])
{
	...
	?><h1 class="<?= $arCurView['TITLE']; ?>" id="<?= $this->GetEditAreaId($arResult['SECTION']['ID']); ?>">
    <a href="<?= $arResult['SECTION']['SECTION_PAGE_URL']; ?>" class="section-head"><?
		echo (
	...
	switch ($arParams['VIEW_MODE']){
		case 'LINE':
			foreach ($arResult['SECTIONS'] as &$arSection){
				...
				></a>
				<h2 class="bx_catalog_line_title"><a href="<?= $arSection['SECTION_PAGE_URL']; ?>"><?= $arSection['NAME']; ?></a><?
				if ($arParams["COUNT_ELEMENTS"]){?>
          <span>(<?= $arSection['ELEMENT_CNT']; ?>)</span><?
				}?>
        </h2><?
				if ('' != $arSection['DESCRIPTION']){
					?><p class="bx_catalog_line_description"><?= $arSection['DESCRIPTION']; ?></p><?
				}?>
		...
		case 'TILE':
			foreach ($arResult['SECTIONS'] as &$arSection){
				...
				?>
        <li id="<?= $this->GetEditAreaId($arSection['ID']); ?>">
          <div class="hold" style="min-height:258px">
            <div class="visual">
              <a href="<?= $arSection['SECTION_PAGE_URL']; ?>" title="<?= $arSection['PICTURE']['TITLE']; ?>">
                <img width="235" height="194" alt="<?=$arSection['PICTURE']['ALT']?>" src="<?=$arSection['PICTURE']['SRC']?>">
              </a>
            </div>
            <div class="block">
              ...
              <p style="text-align:center;"><?= wfGetPluralEnum($arSection['ELEMENT_CNT'],"offers"); ?></p>
            </div>
          </div>
        </li><?
			}       
