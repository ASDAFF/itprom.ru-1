...
//	alert("pbWidth =" + pbWidth + "\n" + "dd3c =" + dd3cWidth);
  ...
  var currSum = parseInt($('#totalPrice').text().replace(' ', '')); //РўРµРєСѓС‰Р°СЏ С†РµРЅР°
  $(".options input").on('change', function () {
    var oldCurrSum = parseInt($('#totalPrice').text().replace(' ', ''));
    var dopSum = 0; 
    $('.options input:checked').each(function () {
      dopSum += parseInt($(this).val());
    });
    var newCurrSum = currSum + dopSum;
    $('#totalPrice').prop("number", oldCurrSum).animateNumber({number: newCurrSum});
  })
