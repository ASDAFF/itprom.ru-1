...
//wfDump($arResult);
 if (!empty($arResult['ITEMS'])):
  ...
  <div class="product-catalog product-catalog04">
    <ul>
	<?
	foreach ($arResult['ITEMS'] as $key => $arItem){
		...
		?>
    <li class="bx_catalog_item" id="<?=$strMainID; ?>">
      <div class="hold">
        <div class="visual">
          <a href="<?=$arItem['DETAIL_PAGE_URL']; ?>">
            <img src="<?=$arItem['PREVIEW_PICTURE']['SRC']?>" width="235" height="194" title="<?=$strTitle?>" alt="<?=$strTitle?>" id="<?=$arItemIDs['PICT']?>"/>
        ...
        <div class="block">
          ...
          <div class="box">
            ...
            <?if($arItem['CAN_BUY']):?>
              <a href="#" class="link-basket" id="<?= $arItemIDs['BUY_LINK']?>"><?=GetMessage("CT_BCS_TPL_MESS_BTN_BUY")?></a>
            <?else:?>
        ...
        <form class="dop_options" method="post" action="" style="margin-top:10px;">
          ...
          <input type="checkbox" id="ch8<?=$key?>" class="fav checkbox" name="my_fav[]" data-count="favCount" value="<?=$arItem["ID"]?>" />
    ...
    $(function(){
      $("body").on("change",".srav",function(){
        ...
        if($(this).is(":checked")){
          ...
          $.get(url, requestData,function(d){});
        }
      });
    });
	</script>
