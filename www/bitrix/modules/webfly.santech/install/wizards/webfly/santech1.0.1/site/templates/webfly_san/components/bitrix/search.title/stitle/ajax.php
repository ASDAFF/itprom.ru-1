	...
	foreach($arCategory["ITEMS"] as $i => $arItem):
	  $curId = "elem-it-".$arItem['PARAM2']."-".$arItem['ITEM_ID'];
        ...
        <div class="li_line info">
			<?=GetMessage("WF_PRODUCER")?>: <?=$arItem["PRODUCT_BRAND"]?><br/>
		  <?=GetMessage("WF_COUNTRY")?>: <?=$arItem["PRODUCT_COUNTRY"]?>
        </div>
					<div class="li_line buy-block">
						...
						<?else:?>
							<a class="add2basket link-basket<?if($arItem["IF_OFFERS"]):?> no-anim<?endif?>" id="<?=$curId?>"><?=GetMessage("ADD2BASKET")?></a>
						<?endif;?>
					</div>
      ...
      <?if($arItem["IF_OFFERS"]):?>
        <script type="text/javascript">
          $("#<?=$curId?>").on("click",function(){
            location.href = "<?=$arItem["URL"]?>";
          });
        </script>
      <?endif;?>
	<?endforeach;?>
