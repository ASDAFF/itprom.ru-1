...
while( $arRes = $rsData->NavNext( true, "f_" ) ){
    ...
    $arExportTime = explode( " ", $f_SETUP["LAST_START_EXPORT"] );
    ...
    if( file_exists( $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/acrit.exportpro/export_{$arRes["ID"]}_run.lock" ) ){
        if( $POST_RIGHT == "W" ){
            $arActions[] = array(
                "ICON" => "unlock",
                "DEFAULT" => true,
    ...
    if( !file_exists( $_SERVER["DOCUMENT_ROOT"]."/bitrix/tools/acrit.exportpro/export_{$arRes["ID"]}_run.lock" ) 
        && ( $f_SETUP["TYPE_RUN"] == "comp" ) ){
            if( $POST_RIGHT == "W" ){
                $runRow = "/bitrix/tools/acrit.exportpro/acrit_exportpro.php?ID=".$arRes["ID"];
