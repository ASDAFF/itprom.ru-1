			...
			foreach($arResult["DB_SOCSERV_USER"] as $key => $arUser)
			{
				...
				<tr class="soc-serv-personal">
					<td class="bx-ss-icons">
						...
						<noindex><a rel="nofollow" class="soc-serv-link" target="_blank" href="<?=$arUser["PERSONAL_LINK"]?>">
						<?endif;?>
						<?=$authID?>
						<?if ($arUser["PERSONAL_LINK"] != ''):?>
							</a></noindex>
					...
					<td class="split-item-actions">
						...
						<noindex><a rel="nofollow" class="split-delete-item" href="<?=htmlspecialcharsbx($arUser["DELETE_LINK"])?>" onclick="return confirm('<?=GetMessage("SS_PROFILE_DELETE_CONFIRM")?>')" title="<?=GetMessage("SS_DELETE")?>"></a></noindex>
						<?endif;?>
					</td>
				</tr>
				<?
