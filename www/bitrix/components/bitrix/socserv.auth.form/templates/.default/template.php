...
<div class="bx-auth-serv-icons">
	...
	?>
	<noindex><a rel="nofollow" title="<?=htmlspecialcharsbx($service["NAME"])?>" href="javascript:void(0)" <?=$onClickEvent?> id="bx_auth_href_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>"><i class="bx-ss-icon <?=htmlspecialcharsbx($service["ICON"])?>"></i></a></noindex>
	<?endforeach?>
</div>
<?endif;?>
<div class="bx-auth">
	<form method="post" name="bx_auth_services<?=$arParams["SUFFIX"]?>" target="_top" action="<?=$arParams["AUTH_URL"]?>">
		...
		<?if($arParams["~FOR_SPLIT"] != 'Y'):?>
			<div class="bx-auth-services">
				<?foreach($arAuthServices as $service):?>
				<div><noindex><a rel="nofollow" href="javascript:void(0)" onclick="BxShowAuthService('<?=$service["ID"]?>', '<?=$arParams["SUFFIX"]?>')" id="bx_auth_href_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>"><i class="bx-ss-icon <?=htmlspecialcharsbx($service["ICON"])?>"></i><?=htmlspecialcharsbx($service["NAME"])?></a></noindex></div>
				<?endforeach?>
			</div>
		<?endif;?>
