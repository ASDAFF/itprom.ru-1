...
<?foreach($arParams["~AUTH_SERVICES"] as $service):?>
	...
	?>
	<noindex><a rel="nofollow" title="<?=htmlspecialcharsbx($service["NAME"])?>" href="javascript:void(0)" <?=$onClickEvent?> id="bx_auth_href_<?=$arParams["SUFFIX"]?><?=$service["ID"]?>"><i class="bx-ss-icon <?=htmlspecialcharsbx($service["ICON"])?>"></i></a></noindex><?endforeach?>
</div>
<?endif;?>
<div class="bx-auth">
